...
$(function () {
    ...
    $("#top_banner_cell_form_set").on('click', '#add_more',function () {
        ...
        var new_form = $('#top_empty_form').html().replace(/__prefix__/g, form_index);
    ...
    $("#top_banner_cell_form_set, #banner_promo_news").on('change', "input[name$='image']", function () {
        var $input = $(this);
        var output = $(this).parent().find("img[class='cinema_preview_banner']")
    ...
    $("#top_banner_cell_form_set, #banner_promo_news").on('click', '#delete_button', function () {
        del_image = $(this).parent().parent().find("img[class='cinema_preview_banner']");
        if (del_image.attr("src") == "/static/cinema/images/empty_form_logo.png") {
            del_image = $(this).parent().parent().find("img");
            del_image.attr("src", "");
            console.log($(this).parent().parent())
            del_true = $(this).parent().parent().find("input[name$='DELETE']");
            del_true[0].checked = true;
            del_container = $(this).parent().parent().parent();
            // del_container.hide();
        } else {
...
// corrected
    ...
    $("#top_banner_cell_form_set, #banner_promo_news").on('click', '#add_button', function () {
...
// ************************************************************************
    ...
    $("#through_banner").on('click', "#id_background_type_0", function(){
        ...
        $("#through_banner #add_button").parent().parent().parent().find('#default_image').hide()
        $("#through_banner #add_button").parent().parent().parent().find('#background_container').show()
    });
    ...
    $("#through_banner").on('click', "#id_background_type_1", function(){
        ...
        $("#through_banner #add_button").parent().parent().parent().find('#background_container').hide()
        $("#through_banner #add_button").parent().parent().parent().find('#default_image').show()
    ...
    $("#through_banner").on('click', '#add_button', function () {
        // console.log($(this).parent().parent())
        $(this).parent().parent().next().find('input[name="delete_image"]').prop("checked", false)
        browse_button = $(this).parent().parent().parent().find('input[type="file"]')
        browse_button.trigger('click');
    });
...
// *****************************************************************************************************************************
    ...
    // // event: add empty form
    // $("#banner_promo_news").on('click', '#add_more_images', function () {
    //     console.log('Click!')
    //     var form_index = $('#id_form-TOTAL_FORMS').val();
    //     var new_form = $('#empty_form_container').html().replace(/__prefix__/g, form_index);
    //     $(new_form).insertBefore($('#add_more_images').parent());
    //     $('#id_form-TOTAL_FORMS').val(parseInt(form_index) + 1);
    // });
    ...
    // console.log($("#banner_promo_news"))

    $("#banner_promo_news").on('click', '#add_more_images', function () {
        var form_index = $('#banner_promo_news #id_form-TOTAL_FORMS').val();
        // console.log('hi!')
        console.log(form_index)
        $('#promo_empty_form input[name$="DELETE"]').hide()
        $('#promo_empty_form input[type=file]').hide()
        var new_form = $('#promo_empty_form').html().replace(/__prefix__/g, form_index);

        $(new_form).insertBefore($('#banner_promo_news #add_more_images').parent());
